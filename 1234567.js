(function() {
    ...
    function categorizeButtons(container) {
        ...
        // Збираємо ВСІ кнопки, крім наших службових (edit-order, folder)
        var allButtons = container.find('.full-start__button').not('.button--edit-order, .button--folder');
        var categories = { online: [], torrent: [], trailer: [], favorite: [], subscribe: [], book: [], reaction: [], other: [] };
        
        allButtons.each(function() {
            var $btn = $(this);
            if (isExcluded($btn)) return; // виключаємо play (але він все одно потрапить у allButtons для збереження)
    ...
    function reorderButtons(container) {
        ...
        // Видаляємо тільки наші додані кнопки (редагування, папки)
        container.find('.button--edit-order, .button--folder').remove();

        // 1. Отримуємо всі кнопки (включно з play)
    ...
    // --- EDITOR DIALOGS (повністю відновлено) ---
    function openEditDialog() {
        if (currentContainer) {
            // Оновлюємо кеш ще раз при відкритті редактора
            var categories = categorizeButtons(currentContainer);
            var allButtons = []
                .concat(categories.online || []).concat(categories.torrent || []).concat(categories.trailer || []).concat(categories.favorite || [])
                .concat(categories.subscribe || []).concat(categories.book || []).concat(categories.reaction || []).concat(categories.other || []);
            allButtonsOriginal = []; 
            allButtons.forEach(function(btn) { if (btn) allButtonsOriginal.push(btn); });
        }
        ...
        var modeBtn = $('<div class="selector viewmode-switch"><div style="text-align: center; padding: 1em;">' + t('custom_interface_plugin_button_view') + ': ' + 
            (currentMode === 'default' ? t('custom_interface_plugin_standard') : currentMode === 'icons' ? t('custom_interface_plugin_icons_only') : t('custom_interface_plugin_with_text')) + '</div></div>');
        modeBtn.on('hover:enter', function() {
            ...
            $(this).find('div').text(t('custom_interface_plugin_button_view') + ': ' + 
                (currentMode === 'default' ? t('custom_interface_plugin_standard') : currentMode === 'icons' ? t('custom_interface_plugin_icons_only') : t('custom_interface_plugin_with_text')));
            if (currentContainer) {
        ...
        var createFolderBtn = $('<div class="menu-edit-list__item menu-edit-list__create-folder selector">' +
            '<div class="menu-edit-list__icon">' + FOLDER_ICON + '</div>' +
            '<div class="menu-edit-list__title">' + t('custom_interface_plugin_create_folder') + '</div>' +
            '</div>');
        ...
        var header = $('<div class="menu-edit-list__header">' +
            ...
            '<div class="menu-edit-list__header-move">' + t('custom_interface_plugin_move') + '</div>' +
            '<div class="menu-edit-list__header-toggle">' + t('custom_interface_plugin_show') + '</div>' +
            '</div>');
        list.append(header);

        function renderItem(type, id, obj) {
            var item;
            if (type === 'folder') {
                item = $('<div class="menu-edit-list__item folder-item" data-type="folder" data-id="' + id + '">' +
                    '<div class="menu-edit-list__icon">' + FOLDER_ICON + '</div>' +
                    '<div class="menu-edit-list__title">' + obj.name + ' <span style="opacity:0.5">(' + obj.buttons.length + ')</span></div>' +
                    ...
                    '<div class="menu-edit-list__delete selector"><svg width="26" height="26" viewBox="0 0 26 26" fill="none"><rect x="1.89" y="1.78" width="21.79" height="21.79" rx="3.5" stroke="currentColor" stroke-width="3"/><path d="M9.5 9.5L16.5 16.5M16.5 9.5L9.5 16.5" stroke="currentColor" stroke-width="3" stroke-linecap="round"/></svg></div>' +
                    '</div>');
                
                item.find('.menu-edit-list__delete').on('hover:enter', function() {
                    deleteFolder(id);
                    var curOrder = getItemOrder();
                    var newOrder = curOrder.filter(function(i){ return !(i.type === 'folder' && i.id === id); });
                    setItemOrder(newOrder);
                    item.remove();
                    Lampa.Noty.show(t('custom_interface_plugin_folder_deleted'));
                });
            } else {
                var btn = obj;
                if (!btn) return null;
                ...
                item = $('<div class="menu-edit-list__item" data-type="button" data-id="' + id + '">' +
                ...
                item.find('.toggle').on('hover:enter', function() {
                    ...
                    if (isNowHidden && idx === -1) hiddenList.push(id);
                    else if (!isNowHidden && idx !== -1) hiddenList.splice(idx, 1);
                    setHiddenButtons(hiddenList);
                });
            }

            // Безпечне переміщення вгору з перевірками
            item.find('.move-up').on('hover:enter', function() {
                var prev = item.prev();
                while (prev.length && (prev.hasClass('menu-edit-list__header') || prev.hasClass('menu-edit-list__create-folder') || prev.hasClass('viewmode-switch'))) {
                    prev = prev.prev();
                }
                if (prev.length && !prev.hasClass('viewmode-switch') && prev[0] !== item[0] && !$.contains(item[0], prev[0]) && !$.contains(prev[0], item[0])) {
                    item.insertBefore(prev);
                    saveItemOrderFromDom(list);
                }
            });
            // Безпечне переміщення вниз
            item.find('.move-down').on('hover:enter', function() {
                var next = item.next();
                if (next.length && !next.hasClass('folder-reset-button') && next[0] !== item[0] && !$.contains(item[0], next[0]) && !$.contains(next[0], item[0])) {
        ...
        if (itemOrder.length) {
            itemOrder.forEach(function(item) {
                if (item.type === 'folder') {
                    var f = folders.find(function(fd) { return fd.id === item.id; });
                    if (f) {
                        list.append(renderItem('folder', f.id, f));
                        processedIds.push(f.id);
                    }
                } else {
                    if (buttonsInFolders.indexOf(item.id) === -1) {
                         var btn = findButton(item.id);
                         if (btn) {
                             list.append(renderItem('button', item.id, btn));
        ...
        var sortedDefault = sortButtonsRaw(allButtonsOriginal);
        sortedDefault.forEach(function(btn) {
            var bid = getButtonId(btn);
            if (processedIds.indexOf(bid) === -1 && buttonsInFolders.indexOf(bid) === -1) {
                var item = renderItem('button', bid, btn);
                if (item) list.append(item);
            }
        });

        folders.forEach(function(f) {
            if (processedIds.indexOf(f.id) === -1) {
                list.append(renderItem('folder', f.id, f));
            }
        });

        var resetBtn = $('<div class="selector folder-reset-button"><div style="text-align: center; padding: 1em;">' + t('custom_interface_plugin_reset_default') + '</div></div>');
        ...
        Lampa.Modal.open({
            ...
            onBack: function() {
                saveItemOrderFromDom(list); 
                Lampa.Modal.close();
                if (currentContainer) reorderButtons(currentContainer);
                if (Lampa.Controller) Lampa.Controller.toggle('full_start');
            }
        });
    }
    ...
    function openCreateFolderDialog() {
        Lampa.Input.edit({
            free: true,
            title: t('custom_interface_plugin_folder_name'),
            nosave: true,
            value: '',
            nomic: true
        }, function(folderName) {
            if (!folderName || !folderName.trim()) {
                Lampa.Noty.show(t('custom_interface_plugin_folder_name'));
                openEditDialog();
                return;
            }
    ...
    function openSelectButtonsDialog(folderName) {
        ...
        var buttonsInFolders = [];
        getFolders().forEach(function(f) { buttonsInFolders = buttonsInFolders.concat(f.buttons); });
        
        var sortedButtons = sortButtonsRaw(allButtonsOriginal);
        ...
        sortedButtons.forEach(function(btn) {
            ...
            var item = $('<div class="menu-edit-list__item">' +
                ...
                '<svg width="26" height="26" viewBox="0 0 26 26" fill="none"><rect x="1.89" y="1.78" width="21.79" height="21.79" rx="3.5" stroke="currentColor" stroke-width="3"/><path d="M7.45 12.97L10.82 16.33L18.13 9.03" stroke="currentColor" stroke-width="3" class="dot" opacity="0" stroke-linecap="round"/></svg>' +
                '</div></div>');
            
            item.find('.menu-edit-list__icon').append(icon);
            item.find('.menu-edit-list__toggle').on('hover:enter', function() {
                var idx = selectedButtons.indexOf(btnId);
                if (idx !== -1) {
                    selectedButtons.splice(idx, 1);
        ...
        var createBtn = $('<div class="selector folder-create-confirm"><div style="text-align: center; padding: 1em;">' + t('custom_interface_plugin_create_folder') + ' "' + folderName + '"</div></div>');
        createBtn.on('hover:enter', function() {
            if (selectedButtons.length < 2) {
                Lampa.Noty.show(t('custom_interface_plugin_min_2_buttons'));
                return;
            }
            var folder = createFolder(folderName, selectedButtons);
            
            var order = getItemOrder();
            order = order.filter(function(i) { return i.type !== 'button' || selectedButtons.indexOf(i.id) === -1; });
            order.push({ type: 'folder', id: folder.id });
            setItemOrder(order);

            Lampa.Modal.close();
            Lampa.Noty.show(t('custom_interface_plugin_folder_created'));
            if (currentContainer) reorderButtons(currentContainer);
            refreshController();
        });
        ...
        Lampa.Modal.open({
            title: t('custom_interface_plugin_select_buttons'),
            html: list,
            size: 'medium',
            scroll_to_center: true,
            onBack: function() {
    ...
    function openFolderEditDialog(folder) {
        ...
        folder.buttons.forEach(function(btnId) {
            ...
            if (btn) {
                var displayName = getButtonDisplayName(btn);
                var icon = btn.find('svg').clone();
                var item = $('<div class="menu-edit-list__item" data-id="' + btnId + '">' +
                    '<div class="menu-edit-list__icon"></div>' +
                    '<div class="menu-edit-list__title">' + displayName + '</div>' +
                    '<div class="menu-edit-list__move move-up selector"><svg width="22" height="14" viewBox="0 0 22 14" fill="none"><path d="M2 12L11 3L20 12" stroke="currentColor" stroke-width="4" stroke-linecap="round"/></svg></div>' +
                    '<div class="menu-edit-list__move move-down selector"><svg width="22" height="14" viewBox="0 0 22 14" fill="none"><path d="M2 2L11 11L20 2" stroke="currentColor" stroke-width="4" stroke-linecap="round"/></svg></div>' +
                ...
                var saveFolderOrder = function() {
                     var newIds = [];
                     list.find('.menu-edit-list__item').each(function() { newIds.push($(this).data('id')); });
                     folder.buttons = newIds;
                     var allFolders = getFolders();
                     var idx = allFolders.findIndex(function(f){ return f.id === folder.id; });
                     if (idx !== -1) { allFolders[idx] = folder; setFolders(allFolders); }
                     updateFolderIcon(folder);
                };

                // Безпечне переміщення вгору
                item.find('.move-up').on('hover:enter', function() {
                    var prev = item.prev();
                    if (prev.length && prev[0] !== item[0] && !$.contains(item[0], prev[0]) && !$.contains(prev[0], item[0])) {
                        item.insertBefore(prev);
                        saveFolderOrder();
                    }
                });
                ...
                item.find('.move-down').on('hover:enter', function() {
                    var next = item.next();
                    if (next.length && next[0] !== item[0] && !$.contains(item[0], next[0]) && !$.contains(next[0], item[0])) {
                        item.insertAfter(next);
                        saveFolderOrder();
                    }
                });
        ...
        Lampa.Modal.open({
            title: t('custom_interface_plugin_folder_order'),
            html: list,
            size: 'small',
            scroll_to_center: true,
            onBack: function() {
