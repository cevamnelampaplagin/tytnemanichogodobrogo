...
(function() {
    ...
    var PLUGIN_VERSION = '1.0.5.2';
    var PLUGIN_AUTHOR = '@Yaroslav_Films moded by cevamnelampaplagin';

    // ==========================================
    // СТИЛІ РЕДАКТОРА (З АДАПТАЦІЄЮ ПІД МОБІЛЬНІ ТА БЕКДРОПИ) 1.0.6.0 — оптимізація продуктивності: hash-індекс метаданих, кешування списків/дублікатів
    // ==========================================
    $('head').append('<style>' +
    ...
    function initCacheEditorActivity() {
        ...
        Lampa.Component.add('cache_editor_grid', function(object) {
            ...
            // КЕШІ ПРОДУКТИВНОСТІ
            // ==========================================
            // hashIndex: hash -> meta, будується ОДИН раз за весь час життя компонента
            // (замість перебору ~1200 варіантів хешу на КОЖЕН нерозпізнаний запис)
            var hashIndex = null;
            // paginating: true лише під час кліку "Показати ще" — сигналізує,
            // що дані НЕ змінились і можна безпечно перевикористати кешований
            // відсортований/відфільтрований список замість повного пересканування
            var paginating = false;
            var groupsCache = { signature: null };
            var keysCache = { signature: null };
            var jsonCache = { signature: null };
            var trashCache = { signature: null };
            var duplicatesCache = { signature: null };
            ...
            // useCache: true лише під час пагінації ("Показати ще") — тоді дані точно
            // не змінювались, і можна уникнути повторного хешування ВСІХ значень
            // localStorage (раніше це відбувалося на кожен виклик buildData, навіть
            // при простому натисканні "показати ще" чи наборі пошукового запиту).
            ...
            // ==========================================
            // buildHashIndex: замість того, щоб на КОЖЕН нерозпізнаний хеш заново
            // перебирати ~1200 комбінацій (сезон×серія×назва) для кожної картки,
            // будуємо зворотній індекс hash -> meta ОДИН раз. Далі розпізнавання
            // будь-якого хешу — це O(1) пошук у мапі замість повторного перебору.
            // Порядок реєстрації співпадає з порядком перевірок у оригінальному
            // алгоритмі (фільм-оригінал, фільм-локальна, серіал-прогрес, серіал-епізод
            // по кожній картці підряд), тому перше знайдене співпадіння для хешу
            // залишається таким самим, як і в оригінальній логіці "return при першому match".
            var buildHashIndex = function() {
                hashIndex = {};
                ...
                var register = function(hashStr, title, subtitle, card) {
                    if (hashIndex[hashStr]) return; // перше співпадіння виграє, як і в оригіналі
                ...
                for (var i = 0; i < uniqueCards.length; i++) {
                    ...
                    var idLabel = card.id || 'Невідомо';

                    var registerMovieTitle = function(t) {
                        if (!t) return;
                    ...
                    var registerSerialProgress = function(nameStr) {
                        if (!nameStr) return;
                        for (var ep = 1; ep <= 50; ep++) {
                            var checkStr = [1, ep, nameStr].join('');
                            register(localHash(checkStr), (card.title || card.name || nameStr) + ' (Серія ' + ep + ')', 'Серіал (ID: ' + idLabel + ')', card);
                        }
                    };
                    registerSerialProgress(origName);
                    registerSerialProgress(localTitle);

                    var registerSerialEpisodes = function(nameStr) {
                        if (!nameStr) return;
                        for (var s = 1; s <= 20; s++) {
                            var delimiter = s > 10 ? ':' : '';
                            for (var e = 1; e <= 60; e++) {
                                var checkStr = [s, delimiter, e, nameStr].join('');
                                register(localHash(checkStr), (card.title || card.name || nameStr) + ' (Сезон ' + s + ', Серія ' + e + ')', 'Серіал (ID: ' + idLabel + ')', card);
                            }
                        }
                    };
            ...
            this.findMetaForHash = function(hash) {
                ...
                // Метод Б: Агресивний текстовий пошук у localStorage (для нетипових плагінів та балансерів).
                // Це вже рідкісний "запасний" шлях — спрацьовує лише для записів, яких
                // не було знайдено в індексі (справді невідомі/orphan записи).
            ...
            this.buildData = function() {
                ...
                if (object.level === 'groups') {
                    var groupsSignature = 'groups|' + q + '|' + sort_mode + '|' + filter_min_kb;
                    var groups, groupBytes, allGroupNames, keysArr;
                    ...
                    if (paginating && groupsCache.signature === groupsSignature) {
                        ...
                        groupsCache = { signature: groupsSignature, groups: groups, groupBytes: groupBytes, allGroupNames: allGroupNames, keysArr: keysArr };
                    }
                    ...
                    var keysSignature = 'keys|' + prefix + '|' + q + '|' + sort_mode + '|' + filter_min_kb;
                    var keyNames, keySizeMap, rawKeyNames;
                    ...
                    if (paginating && keysCache.signature === keysSignature) {
                        ...
                        keysCache = { signature: keysSignature, keyNames: keyNames, keySizeMap: keySizeMap, rawKeyNames: rawKeyNames };
                    }

                    // Розмір усієї групи (завжди по повному переліку групи, незалежно від фільтра розміру)
                    if (!q) {
                        var groupTotalBytes = 0;
                        rawKeyNames.forEach(function(kn) { groupTotalBytes += keySizeMap[kn]; });
                        addControlCard(
                            '📊 Розмір групи «' + prefix + '»',
                ...
                // РІВЕНЬ: JSON-РЕДАКТОР (Таймкоди)
                    ...
                    var jsonSignature = 'json|' + object.storage_key + '|' + q + '|' + sort_mode + '|' + filter_min_kb;
                    var pKeys, jsonSizeMap, dateField;
                    ...
                    if (paginating && jsonCache.signature === jsonSignature) {
                        ...
                        // Пошук за ключем/назвою/підзаголовком. getMeta тепер O(1) завдяки
                        // hashIndex (див. findMetaForHash), тож фільтрація по всьому списку
                        // більше не запускає перебір ~1200 варіантів на кожен запис.
                        if (q) {
                        ...
                        jsonCache = { signature: jsonSignature, pKeys: pKeys, jsonSizeMap: jsonSizeMap, dateField: dateField };
                    }
                ...
                // РІВЕНЬ: КОШИК (відновлення видаленого)
                    ...
                    if (paginating && trashCache.signature === trashSignature) {
                        ...
                        trashCache = { signature: trashSignature, trashArr: trashArr };
                    }
                ...
                // РІВЕНЬ: ДУБЛІКАТИ (однаковий вміст під різними ключами)
                    ...
                    var dupSets = self.findDuplicateSets(paginating);
