...
(function () {
  ...
  var parsersInfo = [
    ...
    },
    {
      ...
      settings: { url: 'https://parserbridge.lampame.v6.rocks/jacred.xyz', key: '', parser_torrent_type: 'jackett' }
    },
    {
      base: 'maxvol_pro',
      name: 'Maxvol.pro (Public)',
      settings: { url: 'https://parserbridge.lampame.v6.rocks/jac.maxvol.pro', key: '1', parser_torrent_type: 'jackett' }
    },
    {
      base: 'otlampa',
      name: 'Всяке різне 6',
    ...
    },
    {
      ...
      settings: { url: 'https://parserbridge.lampame.v6.rocks/http://jacred.torrservera.net', key: '', parser_torrent_type: 'jackett' }
    },
    {
      base: 'jacred_stull',
      name: 'Jacred12 (stull)',
      settings: { url: 'https://parserbridge.lampame.v6.rocks/http://jac.stull.xyz', key: '1', parser_torrent_type: 'jackett' }
    },
    {
      base: 'spawn_pp_ua',
      name: 'Spawnpp-UA (Ukrainian trackers)',
      settings: { url: 'https://parserbridge.lampame.v6.rocks/spawn.pp.ua:59117', key: '2', parser_torrent_type: 'jackett' }
    },
    {
      base: 'localhost',
  ...
  // Додаємо оригінальний хост (без проксі та протоколу) для кожного парсера
  function extractOriginalHost(url) {
    // Видаляємо префікс проксі, якщо він є
    const proxyPrefix = "https://parserbridge.lampame.v6.rocks/";
    let withoutProxy = url;
    if (url.startsWith(proxyPrefix)) {
      withoutProxy = url.slice(proxyPrefix.length);
    }
    // Видаляємо протокол (http:// або https://), якщо є
    const protocolMatch = withoutProxy.match(/^https?:\/\//i);
    if (protocolMatch) {
      withoutProxy = withoutProxy.slice(protocolMatch[0].length);
    }
    // Відсікаємо все після слеша (шлях) – в наших даних шляху немає, але про всяк випадок
    const slashIndex = withoutProxy.indexOf('/');
    if (slashIndex !== -1) {
      withoutProxy = withoutProxy.substring(0, slashIndex);
    }
    return withoutProxy;
  }

  parsersInfo.forEach(function(p) {
    p.originalHost = extractOriginalHost(p.settings.url);
  });
  ...
  /* =========================
   * 4) Перевірки (тепер без проксі, напряму до оригінальних серверів)
   * ========================= */

  // HEALTH candidates – використовуємо originalHost
  function healthUrlCandidates(parser) {
    ...
    var host = parser.originalHost; // без протоколу
    var protos = protocolCandidatesFor(host);

    return protos.map(function (p) { return p + host + path; });
  }
  ...
  // SEARCH candidates (2 стани) – використовуємо originalHost
