(function () {
    ...
    // 1a. Перевірка платформи — плагін працює лише на мобільних пристроях
    // Використовуємо офіційний Lampa.Platform.screen('mobile') — саме так у самому
    // клієнті Lampa визначається "мобільний це пристрій чи ТВ" (враховує touch,
    // співвідношення сторін екрану, Android TV/Google TV UA тощо).
    // Lampa.Platform.is(...) тут НЕ підходить: він звіряє лише збережене значення
    // 'platform' (tizen/webos/android/netcast/orsay/apple/apple_tv/nw/electron) —
    // значення 'mobile' серед них немає.
    function isMobilePlatform() {
        if (window.Lampa && Lampa.Platform && typeof Lampa.Platform.screen === 'function') {
            try {
                return Boolean(Lampa.Platform.screen('mobile'));
            } catch (e) { /* ігноруємо і йдемо до резервної перевірки */ }
        }

        // Резервна перевірка, якщо Lampa.Platform ще не ініціалізований
    ...
    // 2. Стилі (Оновлено: додано агресивне усунення відступів між блоками)
    function applyStyles() {
        ...
        /* ЗОНА 1: Ховаємо пустий заголовок та контейнер значків */
        ...
        css += '.quality-badges-container:empty { display: none !important; margin: 0 !important; min-height: 0 !important; padding: 0 !important; } ';
        
        /* ЗОНА 2: Обнулення відступів між стартовим блоком та описом "Детально" */
        ...
        /* Компактні відступи для інших деталей */
                ...
                if ($details.length) {
                    ...
                    if (Lampa.Storage.get('mobile_interface_quality') && Lampa.Storage.field('parser_use')) {
                        Lampa.Parser.get({ search: movie.title || movie.name, movie: movie, page: 1 }, function (response) {
                            ...
                            // Обов'язковий колбек помилки: без нього Lampa.Parser.get кидає
                            // "onerror is not a function", коли Jackett/Prowlarr/TorrServer
                            // не відповідає, немає посилання, чи стався таймаут.
    ...
    function start() {
        if (!isMobilePlatform()) return; // плагін вимкнено на не-мобільних платформах (ТВ тощо)
