(function() {
    ...
    Lampa.Lang.add({
        ...
        custom_interface_plugin_back: {
            uk: 'Назад',
            ru: 'Назад',
            en: 'Back'
        },
        custom_interface_plugin_no_buttons: {
            uk: 'Немає кнопок',
            ru: 'Нет кнопок',
            en: 'No buttons'
        }
    ...
    function getFolderStructure() {
        ...
        return Lampa.Storage.get('button_folder_structure', defaultStructure);
    }
    ...
    function sortByFolderStructure(buttons) {
            ...
            if (itemId.startsWith('folder_')) {
                ...
                if (buttonMap[itemId]) {
                    ...
                    for (var folderId in folders) {
                        if (folders[folderId].buttons && folders[folderId].buttons.indexOf(itemId) !== -1) {
                            isInFolder = true;
                            break;
                        }
                    }
        ...
        buttons.forEach(function(btn) {
            if (btn) {
                ...
                for (var folderId in folders) {
                    if (folders[folderId].buttons && folders[folderId].buttons.indexOf(id) !== -1) {
                        isInAnyFolder = true;
                        break;
                    }
                }
    ...
    function openEditDialog() {
        ...
        function createFolderItem(folderId, folderData) {
            ...
            item.find('.menu-edit-list__folder-open').on('hover:enter', function() {
        ...
        // Додаємо кнопку створення папки (ВИПРАВЛЕНО)
        ...
        createFolderBtn.on('hover:enter', function() {
            // Використовуємо prompt для введення назви папки
            var folderName = prompt(t('custom_interface_plugin_enter_folder_name'), t('custom_interface_plugin_folder'));
