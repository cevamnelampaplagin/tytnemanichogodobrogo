(function () {
    ...
    // 1. Ініціалізація налаштувань mod 6 fix 150826
    var settings_list = [
        { id: 'mobile_interface_animation', default: false },
        { id: 'mobile_interface_studios', default: false },
    ...
    // 1a. Перевірка платформи
    function isMobilePlatform() {
        if (window.Lampa && Lampa.Platform && typeof Lampa.Platform.screen === 'function') {
            try {
                return Boolean(Lampa.Platform.screen('mobile'));
            } catch (e) { }
        }
        var ua = (navigator.userAgent || navigator.vendor || '').toLowerCase();
        var isTV = /tizen|webos|smarttv|smart-tv|googletv|google tv|mibox|mitv|android tv/.test(ua);
        if (isTV) return false;
        return /android|iphone|ipad|ipod|mobile/.test(ua);
    }

    // 1b. Перевірка "мобільного" розміру екрана незалежно від орієнтації
    ...
    function applyStyles() {
        ...
        // Спільні стилі для бейджів якості/студій (потрібні в обох орієнтаціях)
        var badgeCss = '.quality-badge { height: 1.3em; opacity: 0; animation: qb_in 0.4s ease forwards; display: flex; align-items: center; } ';
        badgeCss += '.studio-logo { height: 1.8em !important; margin-right: 4px; } ';
        badgeCss += '.quality-badge img { height: 100%; width: auto; display: block; } ';
        badgeCss += '.quality-badges-container:empty { display: none !important; margin: 0 !important; min-height: 0 !important; padding: 0 !important; } ';

        // ПОРТРЕТ (<=480px, вузький екран): повне оформлення з фулскрін-постером та центруванням.
        // Розраховане саме на вузьку/високу пропорцію - НЕ застосовувати в альбомній орієнтації.
        css += '@media screen and (max-width: 480px) and (orientation: portrait) { ';
        ...
        // Додано абсолютне позиціонування для коректного накладання картинок слайдшоу
        css += '.full-start-new__poster img { position: absolute !important; top: 0 !important; left: 0 !important; width: 100% !important; height: 100% !important; object-fit: cover !important; ';
        ...
        css += '.full-start-new__title { width: 100%; display: flex; justify-content: center; min-height: auto !important; margin: 0 !important; } ';
        css += '.full-start-new__title:empty { display: none !important; margin: 0 !important; padding: 0 !important; } ';
        css += '.quality-badges-container { display: flex; align-items: center; justify-content: center; gap: 0.6em; margin: 4px 0 !important; flex-wrap: wrap; width: 100%; min-height: auto !important; } ';

        css += '.full-start-new { margin-bottom: 0 !important; padding-bottom: 0 !important; } ';
        css += '.full-start-new__buttons { margin-top: 6px !important; margin-bottom: 5px !important; } ';
        css += '.full-descr { margin-top: 5px !important; padding-top: 0 !important; } ';
        css += '.full-descr__left { margin-top: 0 !important; padding-top: 0 !important; } ';

        css += '.full-start-new__details { margin-bottom: 6px !important; } ';
        ...
        css += '.full-start-new__tagline { font-style: italic; opacity: 0.8; font-size: 0.95em; margin-bottom: 6px !important; color: #fff; } ';
        css += '.full-descr__text { margin-top: 4px !important; } ';

        css += badgeCss;
        css += '} ';

        // АЛЬБОМНА (<=480px висоти, широкий екран): НЕ чіпаємо постер/позиціонування/центрування -
        // рідна двоколонкова верстка Lampa вже коректна, просто акуратно вписуємо бейджі якості.
        css += '@media screen and (max-height: 480px) and (orientation: landscape) { ';
        css += '.quality-badges-container { display: flex; align-items: center; justify-content: flex-start; gap: 0.6em; margin: 4px 0 !important; flex-wrap: wrap; } ';
        css += badgeCss;
        css += '} ';
    ...
    // 6. Логіка завантаження Лого фільму, Якості та Слайдшоу
    function initLogoAndBadges() {
        Lampa.Listener.follow('full', function (e) {
            ...
            if (isMobileScreenSize() && (e.type === 'complite' || e.type === 'complete')) {
                stopSlideshow(); 
                ...
                $.ajax({
                    ...
                    success: function(res) {
                        if (res.logos && res.logos.length > 0) {
                            var logo = res.logos.find(function(l) { return l.iso_639_1 === lang; }) || 
                                       res.logos.find(function(l) { return l.iso_639_1 === 'en'; }) || 
                                       res.logos[0];
                            if (logo) renderLogo(logo.file_path);
                        }

                        if (res.backdrops && res.backdrops.length > 1) {
                            var cleanBackdrops = res.backdrops.filter(function(b) { return b.aspect_ratio > 1.5; });
                            if (cleanBackdrops.length > 0) {
                                var $poster = $render.find('.full-start-new__poster');
                                startPosterSlideshow($poster, cleanBackdrops.slice(0, 15));
                            }
                        }
                    }
                });
                ...
                if ($details.length) {
                    ...
                    if (Lampa.Storage.get('mobile_interface_quality') && Lampa.Storage.field('parser_use')) {
                        Lampa.Parser.get({ search: movie.title || movie.name, movie: movie, page: 1 }, function (response) {
                            ...
                            console.warn('mobile_interface: parser error', error);
                        });
                    }
                }
            }
    ...
    function start() {
        ...
        addSettings();
        initLogoAndBadges();
        setInterval(function () { if (isMobileScreenSize() && window.lampa_settings) window.lampa_settings.blur_poster = false; }, 1000);
    }
