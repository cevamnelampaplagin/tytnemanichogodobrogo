(function () {
    ...
    function isMobilePlatform() {
        // Lampa.Platform.is приймає рядки на кшталт 'mobile', 'android', 'ios', 'tizen', 'webos' тощо.
        // Точний повний список залежить від збірки клієнта Lampa (визначається в самому клієнті,
        // не в цьому репозиторії), тому перевіряємо кілька поширених мобільних варіантів,
        // а не покладаємось лише на один рядок.
        if (window.Lampa && Lampa.Platform && typeof Lampa.Platform.is === 'function') {
            try {
                if (Lampa.Platform.is('mobile')) return true;
                if (Lampa.Platform.is('android') || Lampa.Platform.is('ios')) return true;
                // Явно виключаємо відомі ТВ-платформи, навіть якщо резервна перевірка їх переплутає
                if (Lampa.Platform.is('tizen') || Lampa.Platform.is('webos') || Lampa.Platform.is('tv')) return false;
            } catch (e) { /* Lampa.Platform.is кинув помилку на невідомому рядку — ігноруємо і йдемо далі */ }
        }

        // Резервна перевірка через User-Agent, якщо Lampa.Platform недоступний або нічого не визначив
        var ua = (navigator.userAgent || navigator.vendor || '').toLowerCase();
        var isTV = /tizen|webos|smarttv|smart-tv|googletv|appletv|hbbtv|netcast/.test(ua);
