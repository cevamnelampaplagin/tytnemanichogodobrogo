...
// fixed MAL CORS 120926: Android → official MAL, browser → Shikimori (Jikan dead)
    ...
    function updateRatings(e) {
        ...
        var requestMAL = function() {
            ...
            // Android / native has no CORS → can use official MAL
            var canUseOfficialMAL = (typeof AndroidJS !== 'undefined') ||
                (window.Lampa && Lampa.Platform && (
                    ...
                    Lampa.Platform.screen('tv') === false && /Android/i.test(navigator.userAgent || '')
                ));
            ...
            var tryShikimori = function(titleIdx) {
                ...
                $.ajax({
                    ...
                    dataType: 'json',
                    headers: { 'User-Agent': 'Lampa-MAL-Plugin/1.1' },
                    timeout: 8000,
            ...
            // ---- Official MAL (only when no CORS) ----
            var tryOfficialMAL = function(titleIdx) {
                if (!clientId) {
                    tryShikimori(0);
                    return;
                }
                if (titleIdx >= titles.length) {
                    tryShikimori(0); // fallback
                    return;
                }
                var q = titles[titleIdx];
                $.ajax({
                    url: 'https://api.myanimelist.net/v2/anime?q=' + encodeURIComponent(q) +
                    ...
                    success: function(data) {
                        ...
                        if (anime && anime.mean) {
                            ...
                            tryOfficialMAL(titleIdx + 1);
                        }
                    },
                    error: function() {
                        // CORS / network → switch to Shikimori
                        tryShikimori(0);
                    }
                });
            };
            ...
            if (canUseOfficialMAL && clientId) {
                ...
                // Browser / Tizen / webOS → only Shikimori (no CORS problems)
                tryShikimori(0);
            }
        };
