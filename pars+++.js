(function () {
    ...
    // ===== НОВА ФУНКЦІЯ: скидання списку парсерів =====
    function resetParsersToDefault() {
        // Видаляємо збережений список
        Lampa.Storage.set(STORAGE_PARSERS, null);
        
        // Скидаємо вибрані парсери
        Lampa.Storage.set(STORAGE_PRI_ACT, NO_PARSER);
        Lampa.Storage.set(STORAGE_SEC_ACT, 0);
        
        // Записуємо свіжий DEFAULT_PARSERS
        ...
        try {
            if (Lampa.Noty && typeof Lampa.Noty.show === 'function') {
                Lampa.Noty.show(Lampa.Lang.translate('bat_reset_done'));
            } else if (Lampa.Toast && typeof Lampa.Toast.show === 'function') {
                Lampa.Toast.show(Lampa.Lang.translate('bat_reset_done'));
            } else {
                alert(Lampa.Lang.translate('bat_reset_done'));
            }
        } catch (e) {}
    }
    ...
    function translate() {
        Lampa.Lang.add({
            ...
            bat_reset_parsers_desc: { 
                en: 'Replace saved parsers with the current list from the plugin', 
                uk: 'Замінити збережений список парсерів на актуальний з плагіна', 
                zh: '用插件中的最新列表替换已保存的解析器' 
            },
            bat_reset_done: { 
