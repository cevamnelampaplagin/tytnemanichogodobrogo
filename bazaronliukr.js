...
window.rch_nws[hostkey].Registry = function RchRegistry(client, startConnection) {
  ...
  function rchRun(json, call) {
    if (typeof NativeWsClient == 'undefined') {
      Lampa.Utils.putScript(["http://lampaua.mooo.com/js/nws-client-es5.js?v18112025"], function() {}, false, function() {
        rchInvoke(json, call);
      }, true);
    } else {
      rchInvoke(json, call);
  ...
  function account(url) {
    ...
    if (url.indexOf('token=') == -1) {
      var token = 'h3nwgimk';
      if (token != '') url = Lampa.Utils.addUrlComponent(url, 'token=h3nwgimk');
    }
  ...
  if (false) {
    if (Platform.is('browser') && location.host.indexOf("127.0.0.1") !== -1) {
      ...
      $.get('http://lampaua.mooo.com/player-inner/' + element.url);
      return;
    }
