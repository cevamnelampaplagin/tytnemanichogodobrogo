//orig https://yarikrazor-star.github.io/lmp/yt.js
(function () {
    'use strict';
// опора на український контент + ~фікс розділу фільмів 060926
    ...
    YouTubeAPI.prototype.searchVideos = function(query, pageToken, options) {
        options = options || {};
        ...
        if (this.apiKey) url += '&key=' + this.apiKey;
        if (pageToken) url += '&pageToken=' + pageToken;
        if (this.region) url += '&regionCode=' + this.region;
        url += '&relevanceLanguage=uk';
        // videoDuration: any | short (<4m) | medium (4-20m) | long (>20m)
        if (options.videoDuration) url += '&videoDuration=' + options.videoDuration;
        if (options.order) url += '&order=' + options.order;

        var minDur = (options.minDurationSec !== undefined) ? options.minDurationSec : 61;
        var self = this;
    ...
    function YouTube() {
        ...
        // Категорії: query-based + спеціальні (search/history/url)
        var cats = {
            home:   { title: '🔥 Головна', query: 'популярні відео Україна' },
            search: { title: '🔍 Пошук' },
            history:{ title: '📺 Історія' },
            music:  { title: '🎵 Музика', query: 'українська музика новинки' },
            movies: { title: '🎬 Фільми', query: 'фільм OR серіал українською повний', videoDuration: 'long', minDurationSec: 1200 },
            games:  { title: '🎮 Ігри', query: 'ігри українською' },
            news:   { title: '📰 Новини', query: 'новини України сьогодні' },
            tech:   { title: '💻 Технології', query: 'технології українською' },
            url:    { title: '🔗 Посилання' }
        };
        
        // Якщо є авторизація — додаємо Підписки та Сподобалось на початок
        if (Lampa.Storage.get('youtube_refresh_token')) {
            var authCats = {
                subscriptions: { title: '🔔 Підписки' },
                liked: { title: '👍 Сподобалось' }
            };
            cats = Object.assign(authCats, cats);
        }
        ...
        var listState = {
            ...
            category: { items: [], page: 0, limit: LIMIT_VIDEOS, token: '', isEof: false },
        ...
        this.create = function () {
            ...
            var initialCat = Lampa.Storage.get('youtube_refresh_token') ? 'subscriptions' : 'home';
        ...
        this.handleCategoryClick = function(catKey) {
            ...
            if (catKey === 'search') {
                ...
                // Категорії з query (home, music, movies, games, news, tech)
                viewState = 'category';
                listState.category = { items: [], page: 0, limit: LIMIT_VIDEOS, token: '', isEof: false };
                this.activity.loader(true);
                var catCfg = cats[catKey] || {};
                var q = catCfg.query || catCfg.title;
                var opts = {};
                if (catCfg.videoDuration) opts.videoDuration = catCfg.videoDuration;
                if (catCfg.minDurationSec) opts.minDurationSec = catCfg.minDurationSec;
                if (catCfg.order) opts.order = catCfg.order;
                this.ensureData('category', function(t) { return api.searchVideos(q, t, opts); })
                    .then(function() { self.renderCurrentView(); });
            }
        };
        ...
        this.showSearchInput = function () {
            ...
            Lampa.Input.edit({ value: searchQuery, free: true, nosave: true }, function(val) {
                ...
                if (!val || !val.trim()) { self.handleCategoryClick('home'); return; }
        ...
        this.showUrlInput = function () {
            mainScroll.clear();
            Lampa.Input.edit({ value: '', free: true, nosave: true, title: 'Вставте посилання YouTube' }, function(val) {
                if (!val || !val.trim()) { self.handleCategoryClick('home'); return; }
                var url = val.trim();
                var videoId = null;
                // youtube.com/watch?v=ID | youtu.be/ID | youtube.com/shorts/ID | просто ID
                var m = url.match(/(?:v=|youtu\.be\/|shorts\/|embed\/)([a-zA-Z0-9_-]{11})/) || url.match(/^([a-zA-Z0-9_-]{11})$/);
                if (m) videoId = m[1];
                if (!videoId) {
                    Lampa.Noty.show('Невірне посилання YouTube');
                    return;
                }
                var item = {
                    id: videoId,
        ...
        this.renderCurrentView = function() {
                ...
                if (slice.length) {
                    sections.push({
                        ...
                        onNext: function() { 
                            ...
                            var p;
                            if (stKey === 'category') {
                                var catCfg = cats[currentCategory] || {};
                                var q = catCfg.query || catCfg.title;
                                var opts = {};
                                if (catCfg.videoDuration) opts.videoDuration = catCfg.videoDuration;
                                if (catCfg.minDurationSec) opts.minDurationSec = catCfg.minDurationSec;
                                if (catCfg.order) opts.order = catCfg.order;
                                p = self.ensureData(stKey, function(t) { return api.searchVideos(q, t, opts); });
                            }
            ...
            if (viewState === 'history') {
                ...
                var catTitle = (cats[currentCategory] && cats[currentCategory].title) || currentCategory;
                if (!addSection('category', catTitle)) mainScroll.append($('<div class="yt-empty">Немає відео</div>'));
