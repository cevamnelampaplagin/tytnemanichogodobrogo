...
window.rch_nws[hostkey].Registry = function RchRegistry(client, startConnection) {
  ...
  function account(url) {
    ...
    if (url.indexOf('token=') == -1) {
      var token = 'sqfwtxqz';
      if (token != '') url = Lampa.Utils.addUrlComponent(url, 'token=sqfwtxqz');
    }
...
if (find_voice_url && !find_voice_url.active) {
 ...
 filter.chosen('sort', [sources[balanser].name]);
  ...
  function startPlugin() {
    window.Flownetua_plugin = true;
    var manifst = {
      type: 'video',
      version: '1.6.7',
      name: 'Flownetua (Безкоштовно)',
      description: 'http://kozak.mooo.com (Безкоштовно)',
      component: 'Flownetua',
 ...
 description: 'http://kozak.mooo.com (Безкоштовно)'
      ...
      onContextLauch: function onContextLauch(object) {
        resetTemplates();
        Lampa.Component.add('Flownetua', component);
    ...
    Lampa.Component.add('Flownetua', component); //то же самое
    resetTemplates();
