// === Compatibility Layer for Lampa 2.x / 3.x ===
function removeRating(card){
    if(!card) return;
    if(typeof card.find==='function'){
        card.find('.card__view .card__lmerating').remove();
    }else if(card instanceof HTMLElement){
        var el=card.querySelector('.card__view .card__lmerating');
        if(el) el.remove();
    }
}
  ...
  */
  ...
  function wirePosterBadgesListener() {
    Lampa.Listener.follow('line', function(e) {
      ...
      var cfg = getCfg();
      if (!cfg.enablePosterBadges) {
        e.items.forEach(function(it) { try { removeRating(it.card); } catch (_) {} });
        return;
      }
