...
(function() {
    ...
    // ==========================================
    // СТИЛІ РЕДАКТОРА (З АДАПТАЦІЄЮ ПІД МОБІЛЬНІ ТА БЕКДРОПИ) 1.0.0.2
    // ==========================================
    $('head').append('<style>' +
        ...
        Lampa.Component.add('cache_editor_grid', function(object) {
                ...
                Lampa.Select.show({
                    ...
                    onSelect: function(a) {
                        if (a.id === 'yes') {
                            ...
                            Lampa.Noty.show('Видалено записів: ' + toDelete.length);
                            render_limit = PAGE_SIZE;
                            self.buildData();
                        }
                        Lampa.Controller.toggle('content');
            ...
            this.buildData = function() {
                ...
                if (object.level === 'groups') {
                    ...
                    keysArr.slice(0, render_limit).forEach(function(p) { self.addCard('📁 ' + p, p, groups[p] + ' записів', 'group'); hasItems = true; });
