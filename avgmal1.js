...
// orig from @Yaroslav_Films, mod simkl + average (~) added 050926
    ...
    function updateRatings(e) {
        ...
        var requestMAL = function() {
            ...
            // Prefer the original/Japanese-romaji title first — MAL rarely has
            // localized (e.g. Cyrillic) titles, so a translated title from TMDB
            // will usually fail to match.
            var title = movie.original_title || movie.original_name || movie.title || movie.name;
            if (!title) return;
            var year = ((movie.release_date || movie.first_air_date || '') + '').substring(0, 4);

            var pickBestMatch = function(list) {
                if (!list || !list.length) return null;
                if (!year) return list[0].node;
                var byYear = list.filter(function(a) {
                    var start = a.node && a.node.start_date;
                    return start && start.substring(0, 4) === year;
                });
                return (byYear.length ? byYear[0] : list[0]).node;
            };

            $.ajax({
                url: 'https://api.myanimelist.net/v2/anime?q=' + encodeURIComponent(title) + '&limit=5&fields=id,title,mean,start_date',
                headers: { 'X-MAL-CLIENT-ID': clientId },
                dataType: 'json',
                success: function(data) {
                    var anime = data && data.data ? pickBestMatch(data.data) : null;
                    if (anime && anime.mean) {
                        addRatingBlock(container, 'rate--mal-score', icons.mal, anime.mean, 'mal');
                    }
                }
            });
        };
