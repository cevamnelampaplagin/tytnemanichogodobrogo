...
(function () {
    ...
    //origi https://arst113.github.io/log/NewPhoneMenu.js
    // модифікації вигляду і переклад на українську cevamnelampaplagin 160826
    var defaults = {
    ...
    .navigation-bar__item {
        ...
        background: linear-gradient(155deg, rgba(255,255,255,${supportsBackdropFilter() ? '0.22' : '0.16'}) 0%, rgba(255,255,255,0.05) 55%, rgba(255,255,255,0.10) 100%);
        border: 1px solid rgba(255,255,255,0.22);
        border-radius: 16px;
        box-shadow: 0 4px 14px rgba(0,0,0,0.25), inset 0 1px 1px rgba(255,255,255,0.45), inset 0 -6px 10px -6px rgba(0,0,0,0.25);
        ...
        position: relative !important;
        isolation: isolate;
        ${supportsBackdropFilter() ? 'backdrop-filter: blur(18px) saturate(180%); -webkit-backdrop-filter: blur(18px) saturate(180%);' : ''}
        min-width: 0 !important;
        max-width: 100% !important;
    }
    /* Specular highlight sweep across the top of the glass, like light catching a curved lens */
    .navigation-bar__item::before {
        content: '';
        position: absolute;
        inset: 0;
        border-radius: inherit;
        background: linear-gradient(200deg, rgba(255,255,255,0.55) 0%, rgba(255,255,255,0) 32%);
        opacity: 0.9;
        pointer-events: none;
        z-index: 1;
    }
    .navigation-bar__item > * { position: relative; z-index: 2; }
    body.glass--style .navigation-bar__item {
        background: linear-gradient(155deg, rgba(255,255,255,0.22) 0%, rgba(255,255,255,0.05) 55%, rgba(255,255,255,0.10) 100%) !important;
        box-shadow: 0 4px 14px rgba(0,0,0,0.25), inset 0 1px 1px rgba(255,255,255,0.45), inset 0 -6px 10px -6px rgba(0,0,0,0.25) !important;
        border: 1px solid rgba(255,255,255,0.22) !important;
    }
    .navigation-bar__item:hover, .navigation-bar__item.active {
        background: linear-gradient(155deg, rgba(255,255,255,0.34) 0%, rgba(255,255,255,0.10) 55%, rgba(255,255,255,0.16) 100%);
        border-color: rgba(255,255,255,0.4);
        transform: scale(1.06);
        box-shadow: 0 6px 22px rgba(0,0,0,0.3), 0 0 22px rgba(120,170,255,0.25), inset 0 1px 1px rgba(255,255,255,0.55), inset 0 -6px 10px -6px rgba(0,0,0,0.2);
        z-index: 1;
    }
    body.glass--style .navigation-bar__item:hover, body.glass--style .navigation-bar__item.active {
        background: linear-gradient(155deg, rgba(255,255,255,0.34) 0%, rgba(255,255,255,0.10) 55%, rgba(255,255,255,0.16) 100%) !important;
        border-color: rgba(255,255,255,0.4) !important;
        box-shadow: 0 6px 22px rgba(0,0,0,0.3), 0 0 22px rgba(120,170,255,0.25), inset 0 1px 1px rgba(255,255,255,0.55) !important;
    }
    ...
    .phone-menu-picker-modal {
        background: linear-gradient(180deg, rgba(255,255,255,0.12) 0%, rgba(255,255,255,0.03) 100%), rgba(30,30,36,0.55);
        ${supportsBackdropFilter() ? 'backdrop-filter: blur(28px) saturate(180%); -webkit-backdrop-filter: blur(28px) saturate(180%);' : ''}
        border: 1px solid rgba(255,255,255,0.22);
        padding: 12px; border-radius: 20px; max-width: 96%; max-height: 88vh; overflow: hidden;
    ...
    .phone-menu-picker-grid .picker-item:hover {
        background: linear-gradient(155deg, rgba(255,255,255,0.22) 0%, rgba(255,255,255,0.06) 100%);
    ...
    // ==================== ЗБІР ПУНКТІВ ЛІВОГО МЕНЮ (без розгортання спрайтів) ====================
    function collectMenuSections(){
        ...
        function add(el){
            ...
            if(name && (name.indexOf('Редагувати') !== -1 || name.indexOf('Налаштування') !== -1)) return;
            seen[key] = true;
            ...
            if(!svg) return;
            // Не розвертаємо <use>, залишаємо як є
    ...
    function showIconPicker(position, div, iconEl, labelEl, defaultAction, defaultSvg, defaultName){
        ...
        title.textContent = 'Налаштування кнопки';
        ...
        function renderOptionsGrid(grid, options, position, div, iconEl, labelEl, overlay, defaultAction, defaultSvg, defaultName){
                ...
                if(opt.action !== '_'){
                    (function(o, a, s, n){
                        item.addEventListener('click', function(){
                            // Зберігаємо іконку як є (без розгортання)
                            div.setAttribute('data-action', a);
                            localStorage.setItem('bottom_bar_' + position + '_action', a);
                            iconEl.innerHTML = s; // використовуємо вихідний SVG
            ...
            reset.textContent = 'Скинути на стандарт';
            reset.addEventListener('click', function(){
                div.setAttribute('data-action', defaultAction);
                localStorage.removeItem('bottom_bar_' + position + '_action');
                iconEl.innerHTML = defaultSvg; // використовуємо стандартну іконку як є
    ...
    function addItem(position, defaultAction, defaultSvg, defaultName){
        ...
        iconDiv.innerHTML = savedSvg; // вставляємо іконку як є (з <use>)
        ...
        // Довге натискання для налаштування
        var timer;
        function start(){
