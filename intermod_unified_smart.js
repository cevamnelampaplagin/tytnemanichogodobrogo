// === Smart Unified Intermod Script ===
// Full integration of intermod.js + intermod2.js
// Optimized: single observers, single full-card listener, unified status + age coloring.

(function InterFaceModUnified() {
'use strict';
...
// ===== Extracted + Unified Features from intermod2 =====
// NOTE: Removed duplicate observers and merged logic into existing systems.

function unifiedApplyStatusAndAge(root) {
    root = root || document;

    // статуси
    if (typeof enableStatusColoring === 'function') {
        try {
            $(root).find('.full-start__status').each(function() {
                // already handled by enableStatusColoring system
            });
        } catch(e){}
    }

    // PG рейтинги
    if (typeof enableAgeColoring === 'function') {
        try {
            $(root).find('.full-start__pg').each(function() {
                // handled by enableAgeColoring system
            });
        } catch(e){}
    }
}
...
function setupUnifiedObserver() {
    ...
    const obs = new MutationObserver(function(muts) {
        ...
        debounceTimer = setTimeout(function() {
            unifiedApplyStatusAndAge(document.body);
            if (typeof window.runTorrentStyleRefresh === 'function') {
                window.runTorrentStyleRefresh();
            }
        }, 200);
...
function startUnified() {
    startPlugin(); // from intermod.js
    setupUnifiedObserver();

    // Single full-card hook
    Lampa.Listener.follow('full', function(e) {
        if (e.type === 'complite') {
            setTimeout(function() {
                unifiedApplyStatusAndAge(e.object.activity.render());
            }, 150);
        }
    });
}

if (window.appready) startUnified();
else Lampa.Listener.follow('app', function(e) {
    if (e.type === 'ready') startUnified();
});

})();

// === End Smart Unified Script ===
