...
// + user CORS proxies (comma-separated) 120926 v1.2
    ...
    function updateRatings(e) {
        ...
        var requestMAL = function() {
            ...
            // Build proxied URL. Supports both styles:
            //   https://corsproxy.io/?          → append encoded target
            //   https://api.allorigins.win/raw?url= → append encoded target
            var buildProxyUrl = function(proxyBase, targetUrl) {
                if (!proxyBase) return targetUrl;
                var base = proxyBase;
                if (base.indexOf('?') === -1 && base.slice(-1) !== '/') base += '/';
                // most public proxies expect the full target URL (often encoded)
                if (base.indexOf('url=') !== -1 || base.indexOf('?') !== -1) {
