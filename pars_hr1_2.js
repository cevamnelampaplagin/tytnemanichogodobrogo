(function () {
    ...
    // original url = https://ko31k.github.io/LMP/plugins/Parsers.js
    ...
    var DEFAULT_PARSERS = [
        ...
        { base: 'alco2', shortName: 'alcoV2', name: 'alco v2 (13+)', url: 'https://tv.alcopa.cc/api/v2.0/indexers/all/results?title=', settings: { key: '', parser_torrent_type: 'jackett' } },
    ...
    // ===== ПОВНИЙ СКИД: усі ключі, які коли-небудь чіпав цей плагін =====
    // Власні ключі плагіна (namespaced) + "стандартні" поля Lampa (jackett_/prowlarr_...),
    // які плагін сам записує при виборі парсера — їх теж чистимо, щоб стан був такий,
    // ніби плагін щойно поставили заново з GitHub.
    var ALL_PLUGIN_KEYS = [
        STORAGE_PARSERS,        // ps_list_combo_v4.3 - збережений каталог парсерів
        STORAGE_PRI_ACT,        // bat_url_two - вибраний основний парсер
        STORAGE_SEC_ACT,        // ps_active_sec_v4.3 - вибраний додатковий парсер
        'parser_use_proxy',
        'parser_proxy_target',
        'jackett_url',
        'jackett_url_two',
    ...
    function wipeAllPluginStorage() {
        ALL_PLUGIN_KEYS.forEach(function (key) {
            try {
                // 1) Official Lampa API (preferred)
                if (Lampa.Storage && typeof Lampa.Storage.remove === 'function') {
                    Lampa.Storage.remove(key);
                }
            } catch (e) {}
            try {
                // 2) Force-clear via set(null / '') — some Lampa builds keep the key after remove()
            ...
            try {
                // 3) Direct localStorage — guarantees the key is gone from the browser/TV store
    ...
    function removeInjectedStyle() {
        ...
        $('style').filter(function () {
            return this.textContent && this.textContent.indexOf('.bat-parser-modal') !== -1;
        }).remove();
        window.__bat_parser_modal_style__ = false;
    }
    ...
    // Повний, "як після перевстановлення" скид: стирає ВСІ ключі плагіна (включно зі
    // стандартними jackett_url/prowlarr_url/... полями, які він сам заповнює), чистить
    // кеш перевірок і UI-стилі, і накатує свіжий DEFAULT_PARSERS з поточного файлу плагіна.
    function resetParsersToDefault() {
        // 1. Стираємо буквально всі ключі, які плагін коли-небудь записував
        //    (Lampa.Storage.remove + set(null/'') + localStorage.removeItem)
        wipeAllPluginStorage();

        // 2. Скидаємо кеш перевірок здоров'я/пошуку - він міг посилатись на старі парсери
        cache.data = {};

        // 3. Прибираємо вставлений <style>, щоб він вставився заново "з нуля"
        removeInjectedStyle();

        // 4. Явно обнуляємо стандартні поля парсера Lampa (після wipe вони можуть
        //    ще "світитися" в UI, якщо Settings кешує значення)
        try {
            ...
            Lampa.Storage.set('parser_use_proxy', false);
            Lampa.Storage.set('parser_proxy_target', 'both');
        } catch (e) {}

        // 5. Записуємо свіжий DEFAULT_PARSERS (так, як він є в поточному файлі плагіна)
        ...
        // 6. Оновлюємо весь UI, що відображає стан плагіна
        updateSelectedLabelInSettings();
        updateStandardFieldsUI();
        try { Lampa.Settings.update(); } catch (e) {}

        var filterBtn = $('.filter--parser');
        if (filterBtn.length) updateBtnName(filterBtn);

        // 7. Повідомлення
        notify('bat_reset_done');
    }

    // Питає підтвердження перед повним скидом (бо він стирає і ручні jackett/prowlarr
    // налаштування, не тільки список парсерів цього плагіна), і лише тоді виконує його.
    function confirmHardReset(onConfirm) {
        var text = Lampa.Lang.translate('bat_reset_confirm');
    ...
    function translate() {
        Lampa.Lang.add({
            ...
            // ===== НОВІ ПЕРЕКЛАДИ =====
            bat_reset_parsers: { 
                en: 'Update actual parsers', 
                uk: 'Оновити актуальні ключі', 
                zh: '更新解析器列表' 
            },
            bat_reset_parsers_desc: { 
                en: 'Wipe every storage key this plugin uses (parser list, selection, proxy settings, and the jackett/prowlarr fields it fills in) and reload the fresh default list, as if just reinstalled', 
                uk: 'Стерти всі ключі сховища, які використовує цей плагін (список парсерів, вибір, налаштування проксі, поля jackett/prowlarr), і завантажити свіжий список за замовчуванням — так, ніби плагін щойно перевстановили', 
                zh: '清除该插件使用的所有存储键（解析器列表、选择、代理设置以及它填写的 jackett/prowlarr 字段），并重新加载默认列表，如同刚重新安装一样' 
            },
            bat_reset_done: { 
                en: 'Parsers list updated', 
                uk: 'Список парсерів оновлено', 
                zh: '解析器列表已更新' 
            },
            bat_reset_confirm: {
                en: 'This will erase ALL data saved by this plugin — the parser catalog, your primary/secondary selection, proxy settings, and the jackett/prowlarr URL & key fields — and reload the fresh default list from the plugin file. This cannot be undone. Continue?',
                uk: 'Це видалить УСІ дані, збережені цим плагіном — каталог парсерів, вибір основного/додаткового, налаштування проксі та поля URL/ключ jackett/prowlarr — і завантажить свіжий список за замовчуванням з файлу плагіна. Це незворотньо. Продовжити?',
                zh: '这将清除该插件保存的所有数据——解析器目录、主/副选择、代理设置以及 jackett/prowlarr 的网址和密钥字段——并重新加载插件文件中的默认列表。此操作不可撤销，是否继续？'
            }
        });
    }
    ...
    function openParserModal() {
        ...
        var btnReset  = $("<div class='bat-parser-modal__action selector'></div>").text(Lampa.Lang.translate('bat_reset_parsers'));

        actions.append(btnHealth).append(btnSearch).append(btnReset);
        ...
        function runHealthUI() {
            ...
            return runHealthChecks(getParsers()).then(function (map) { applyMapToList(map); notifyDone(); });
        }
        ...
        function runSearchUI() {
            ...
            return runDeepSearchChecks(getParsers()).then(function (map) { applyMapToList(map); notifyDone(); });
        }

        btnHealth.on('hover:enter', runHealthUI);
        btnSearch.on('hover:enter', runSearchUI);

        // ===== КНОПКА ОНОВЛЕННЯ (повний скид, з підтвердженням) =====
        btnReset.on('hover:enter', function () {
            confirmHardReset(function () {
            resetParsersToDefault();
            
            // Очищаємо і перебудовуємо список у відкритому вікні
            list.empty();
            ...
            // Новий актуальний список
            getParsers().forEach(function (p) {
                var item = buildParserItem(p.base, p.name);
                item.on('hover:enter', function () {
                    Lampa.Storage.set(STORAGE_PRI_ACT, p.base);
                    applySelectedParser(p.base);
            ...
            // Автоматично перевіряємо доступність після оновлення
            runHealthUI();
            });
        });
        ...
        Lampa.Select.show({
            ...
            onSelect: function (item) {
                if (item.add) {
                    ...
                    // Повний скид як «перевстановлення» — ті самі ключі, що й кнопка «Оновити актуальні ключі»
                    confirmHardReset(function () {
                        resetParsersToDefault();
                        updateBtnName(btn);
                        Lampa.Controller.toggle(enabled);
