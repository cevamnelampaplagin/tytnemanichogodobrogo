(function () {
    ...
    function Collection(data) {
      ...
      this.build = function () {
        ...
        this.item.find('.luno-collection-card__items').text(data.items_count + ' Карток');
        ...
        if (data.views && data.views != 0) {
          this.item.find('.luno-collection-card__views').text(Lampa.Utils.bigNumberToShort(data.views) + ' переглядів');
        }
    ...
    var collections = [{
      ...
      hpu: 'week',
      title: 'Популярні за тиждень'
    }, {
      hpu: 'month',
      title: 'Популярні за місяць'
    }, {
      hpu: 'all',
      title: 'Усі колекції'
    }];
