	...
	function startSwitchMouse() {
		...
		if(Lampa.Platform.is("android")){
			if(!Lampa.Storage.get("weapon_choised", "false")){
				...
				let btns = [
					{
						name: "Пульт без мишки",
						select: dpad
					},
					{
						name: "Миш / Пульт с мишкою",
						select: mouse
					},
					{
						name: "Тачскрін / Сенсор",
						select: touch
					}
				]
			...
			var addToSettings = function addToSettings() {
				...
				Lampa.SettingsApi.addParam({
					...
					field: {
						name: 'Скинути тип керування',
						description: 'Скинути тип керування выбраний раніше'
					},
					onRender: function(i){
						i.addClass('weapon_choised')
						i.unbind('hover:enter').on('hover:enter', function () {
							Lampa.Storage.set('weapon_choised', false);
							Lampa.Noty.show('Тип керування скинутий')
							setTimeout(function(){
									window.location.reload()
								}, 3000)
