...
(function() {
    ...
    // ==========================================
    // СТИЛІ РЕДАКТОРА (З АДАПТАЦІЄЮ ПІД МОБІЛЬНІ ТА БЕКДРОПИ) 1.0.1.0 — додано відображення розміру кешу
    // ==========================================
    $('head').append('<style>' +
        ...
        Lampa.Component.add('cache_editor_grid', function(object) {
                ...
                Lampa.Select.show({
                    ...
                    onSelect: function(a) {
                        if (a.id === 'yes') {
                            ...
                            Lampa.Noty.show('Видалено записів: ' + toDelete.length + ' (звільнено ' + formatBytes(freedBytes) + ')');
                            render_limit = PAGE_SIZE;
                            self.buildData();
                        }
                        Lampa.Controller.toggle('content');
            ...
            this.buildData = function() {
                ...
                if (object.level === 'groups') {
                    ...
                    // Загальний обсяг усього локального сховища
                    if (!q) {
                        var totalStorageBytes = 0;
                        for (var gKey in groupBytes) totalStorageBytes += groupBytes[gKey];
                        addControlCard(
                            '📊 Загальний розмір кешу',
                            formatBytes(totalStorageBytes) + ' • ' + localStorage.length + ' записів у ' + keysArr.length + ' групах',
                            function() { self.buildData(); },
                            { wide: true }
                        );
                    }

                    // Сортуємо групи за розміром, найважчі — першими, щоб було видно, що "їсть" пам'ять
                    keysArr.sort(function(a, b) { return groupBytes[b] - groupBytes[a]; });
                    ...
                    for (var j = 0; j < localStorage.length; j++) {
                        ...
                        if (keyName && (keyName.split('_')[0] === prefix || keyName === prefix)) keyNames.push(keyName);
                    }

                    // Розмір усієї групи
                    if (!q) {
                        var groupTotalBytes = 0;
                        keyNames.forEach(function(kn) { groupTotalBytes += getKeyByteSize(kn); });
                        addControlCard(
                            '📊 Розмір групи «' + prefix + '»',
                    ...
                    if (q) keyNames = keyNames.filter(function(keyName) { return keyName.toLowerCase().indexOf(q) !== -1; });

                    // Найважчі записи — першими
                    keyNames.sort(function(a, b) { return getKeyByteSize(b) - getKeyByteSize(a); });

                    var totalKeys = keyNames.length;
                    keyNames.slice(0, render_limit).forEach(function(keyName) {
                        var val = localStorage.getItem(keyName) || '';
                        var sizeLabel = formatBytes(getKeyByteSize(keyName));
                ...
                // РІВЕНЬ: JSON-РЕДАКТОР (Таймкоди)
                    ...
                    if (totalJson > 0) {
                        pKeys.slice(0, render_limit).forEach(function(k) {
                            ...
                            var itemSize = formatBytes(byteSize(JSON.stringify(item)));
                        ...
                        onSelect: function(a) {
                            if (a.id === 'del') {
                                if (type === 'group') {
                                    ...
                                    toDel.forEach(function(k) { freedGroupBytes += getKeyByteSize(k); localStorage.removeItem(k); });
                                    Lampa.Noty.show('Групу ' + rawId + ' видалено (звільнено ' + formatBytes(freedGroupBytes) + ')');
                                } else if (type === 'json_item') {
