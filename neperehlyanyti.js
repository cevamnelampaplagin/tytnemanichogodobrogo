...
//orig https://temcher.github.io/lampa/unwatched-series.js
    var api = factory();
    ...
    function openFallback(lampa) {
        modelForLampa(lampa).then(function (model) {
            ...
            if (!items.length) {
                if (lampa.Noty && lampa.Noty.show) lampa.Noty.show('Немає непереглянутих серій');
                return;
            }
    ...
    function register(lampa) {
        ...
        try {
            if (lampa.Lang && lampa.Lang.add) {
                lampa.Lang.add({
                    unwatched_series_next: { ru: 'Наступні серії', en: 'Next episodes' },
                    unwatched_series_recent: { ru: 'Вийшли нещодавно', en: 'Recently released' },
                    unwatched_series_tracking: { ru: 'Дивлюсь', en: 'Watching' },
                    unwatched_series_title: { ru: 'Непереглянуті серії', en: 'Unwatched series' }
                });
            }
