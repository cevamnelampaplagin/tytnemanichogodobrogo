(function () {
    ...
    function fetchAll() {
        ...
        var alertsP = fetchAlerts();

        var newsP = showNews ? fetchNews() : Promise.resolve([]);

        Promise.all([rateP, fuelP, alertsP, newsP])
            .then(function (results) {
                // порядок: курси → паливо → тривоги → новини
                var rates = [].concat(results[0] || [], results[1] || [], results[2] || []);
                var news  = results[3] || [];
    ...
    function registerSettings() {
        ...
        Lampa.SettingsApi.addParam({
            ...
            field: {
                ...
                description: 'Активні тривоги по областях/районах (Trivoga / air-alert.in.ua)'
            },
            onChange: function () { scheduleRefresh(); }
        });
        ...
        Lampa.SettingsApi.addParam({
            ...
            field: {
                name: 'Ховати постійні тривоги',
                description: 'Не показувати Крим, Луганську тощо з 2022 року'
            },
            onChange: function () { scheduleRefresh(); }
        });
        ...
        addTextInput('rss_alerts_regions', 'Фільтр областей (тривоги)', 'Напр: Київ, Харків, Дніпро (порожньо = усі активні)');
        addTextInput('rss_alerts_api_key', 'API-ключ Trivoga (опційно)', 'Якщо порожньо — використовується вбудований');
                    ...
                    item.on('hover:enter', function () {
                        Lampa.Input.edit(
                            ...
                            function (val) {
                                ...
                                // Одразу пробуємо завантажити й розпарсити фід, щоб було видно
                                // ЧОМУ джерело не працює, а не просто тишу в стрічці.
        ...
        Lampa.SettingsApi.addParam({
            ...
            field: {
                name: 'Скільки новин з одного джерела',
                description: '5 / 10 / 25 або максимум (50) з кожного RSS-фіду'
            },
            onChange: function () { scheduleRefresh(); }
        });
