(function () {
    'use strict';
// orig from @Yaroslav_Films, mod simkl + average (~) added + mal (jikan) 120926
        ...
        var requestMAL = function() {
            ...
            var pickBestMatch = function(list) {
                ...
                var byYear = list.filter(function(a) {
                    var aired = a.aired && a.aired.prop && a.aired.prop.from && a.aired.prop.from.year;
                    return aired && String(aired) === year;
                });
                return byYear.length ? byYear[0] : list[0];
            };

            $.getJSON('https://api.jikan.moe/v4/anime?q=' + encodeURIComponent(title) + '&limit=5', function(data) {
                var anime = data && data.data ? pickBestMatch(data.data) : null;
                if (anime && anime.score) {
                    addRatingBlock(container, 'rate--mal-score', icons.mal, anime.score, 'mal');
                }
            });
        };
